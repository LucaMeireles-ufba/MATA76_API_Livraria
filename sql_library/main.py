...
@app.post("/v1/authors/", response_model=schemas.Author)
...
@app.put("/v1/book/", response_model = schemas.Book)
...
@app.delete("/v1/book/", response_model = schemas.Book)
...
@app.get("/v1/customers/", response_model = list[schemas.Customer])
def read_customers(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return crud.get_customers(db, skip=skip, limit=limit)

@app.get("/v1/customer/{customer_id}", response_model = schemas.Customer)
def read_customer(customer_id: int, db: Session = Depends(get_db)):
    return crud.get_customer(db, customer_id=customer_id)

@app.post("/v1/customers/", response_model = schemas.Customer)
def create_customer(customer: schemas.CustomerCreate, db: Session = Depends(get_db)):
    return crud.create_customer(db, customer=customer)

### EMPRÉSTIMOS ###

@app.get("/v1/loans/", response_model = list[schemas.Loan])
def read_loans(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return crud.get_loans(db, skip=skip, limit=limit)

@app.get("/v1/loan/{loan_id}", response_model=schemas.Loan)
...
@app.post("/v1/loans/", response_model=schemas.Loan)
